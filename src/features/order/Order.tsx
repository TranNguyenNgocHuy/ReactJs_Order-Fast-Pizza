  ...
  return (
    <div className='space-y-8 px-4 py-6'>
      ...
      <ul className='divide-y divide-stone-200 border-b border-t'>
        {cart.map((item) => (
          <OrderItem
            ...
            ingredients={fetcher.data?.find((el: Order) => Number(el.id) === item.pizzaId)?.ingredients ?? []}
          />
        ))}
      </ul>
      ...
      <div className='space-y-2 bg-stone-200 px-6 py-5'>
        ...
        {priority && (
          <p className='text-sm font-semibold text-stone-600'>Price priority: {formatCurrency(priorityPrice)}</p>
        )}
